...
export type { RateLimiterResult } from './strategy';
...
export class RateLimiter {
  ...
  private async handleRedisError<T>(promise: Promise<T>): Promise<T | RateLimiterResult> {
    try {
  ...
  async hit(identifier: string): Promise<RateLimiterResult> {
    return this.handleRedisError(this.strategy.limit(identifier)) as Promise<RateLimiterResult>;
  }
